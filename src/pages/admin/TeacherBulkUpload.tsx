...
export const TeacherBulkUpload: React.FC<TeacherBulkUploadProps> = ({ onSuccess }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        const results = await db.bulkImportTeachers(teachersToImport);
        toast.success(`Successfully imported ${results.length} teachers`);
  ...
  return (
    <div className="bg-white p-6 rounded-xl border border-school-cream-200">
      <h3 className="text-xl font-bold text-gray-800 mb-4">Bulk Teacher Upload</h3>
      <p className="text-gray-600 mb-6 font-medium">
        Upload a CSV file with headers: <code className="bg-gray-100 px-2 py-1 rounded">surname, other_names, department, gender, title, position_rank</code>
      </p>
